...
int main(int ac, char *av[])
{
	...
	int    sock_id, sock_fd;            	/* the socket and fd (dont need fd here)  */
	char   buffer[BUFSIZ];              	/* to receive message */
	int    n_read;                      	/* for message length */

	if ( ac != 3 ) 
	{
		printf("Error: Not enough argument. Usage: ./rls IP_addr ~directory ");
		exit(1);
	}
	...
	hp = gethostbyname( av[1] );            /* lookup host's ip #   */ // Use domain name or the IP
	if (hp == NULL) 
		oops(av[1]);            			/* or die               */
	...
	// Write the file directory to the Server
	if (write(sock_id, av[2], strlen(av[2])) == -1) 
		oops("write");
	if (write(sock_id, "\n", 1) == -1 ) // And add a newline
		oops("write");

	// Read the "ls" data from the server and print it to stdout
